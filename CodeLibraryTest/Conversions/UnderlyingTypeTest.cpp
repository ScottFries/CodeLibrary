#include "Conversions/UnderlyingType.hpp"
...
TEST(UnderlyingTypeTest, uint8)
{
    auto test_value = [](Month m, std::uint8_t val) {
        auto m_val = toUType(m);
        ASSERT_EQ(typeid(m_val), typeid(std::uint8_t));
        ASSERT_EQ(m_val, val);
    };
...
TEST(UnderlyingTypeTest, uint64)
{
    auto test_value = [](ByteSize b, std::uint64_t val)
    {
        auto m_val = toUType(b);
        ASSERT_EQ(typeid(m_val), typeid(std::uint64_t));
        ASSERT_EQ(m_val, val);
    };
